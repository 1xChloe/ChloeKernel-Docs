...
Chain order is free but the **gate order is fixed**: once → cooldown → count, whatever order you chained them. `where` filters on the event args, `count(n)` fires every nth surviving match, `once()` fires a single time **ever** per key — so combining it with `count`/`cooldown` still fires at most once (for "repeated but throttled" use `cooldown` alone) — and `cooldown(seconds)` drops matches inside the window, all counted **per key**, defaulting to the first event arg (the session, in every kit's publish convention; override with `:key(fn)`). `run(action)` arms the rule and locks the chain (stages after it error). Counters hold weak keys so departed sessions drop their state, and actions run through `task.spawn` — a throwing rule never breaks the bus.
...
The first event arg must resolve to the session (override with `SessionOf` per achievement, for topics that lead with something else). Progress and unlocks store under `profile.Data.Achievements` (rename via `Field`), `Reward` runs once at unlock, and post-unlock events are ignored — no re-grants. Events landing before the profile loads count into session data and merge on `Kernel.ProfileLoaded` (unlocks carry over, the higher count per id wins — the same rule QuestKit uses), so a kill in the first second of a session is never dropped. Bus: `Achievement.Progress(session, id, count, target)` on every step for toasts and bars, `Achievement.Unlocked(session, id)` once. Like quests, achievements count server-published topics: there is nothing for a client to spoof.
