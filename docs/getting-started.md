...
Boot scripts are pre-wired: pressing Play runs the spec suites and boots both kernels. Game code goes in the `Bootstrap` modules (`src/Server/Bootstrap.luau`, `src/Client/Bootstrap.luau`) — each receives its kernel after boot, before start. See [Run code when a player joins or leaves](cookbook.md#run-code-when-a-player-joins-or-leaves) for the basic shape.
